...
$query = $pdo->prepare('SELECT id_c, comment.author, username, comment.content, comment.date_published, parent_message, comment.signaled, id_m, message.title FROM comment
    ...
    WHERE category = ? AND comment.signaled = TRUE') ;
  ...
  <tbody>
    ...
    <tr class="<?= $comment['signaled'] == true ? 'table-warning' : '' ; ?>">
      ...
      <td scope="col">
        ...
        <button type="button" class="btn btn-success btn-sm" onclick="unsignalC(<?= $comment['id_c'] ; ?>, 'signaled')">Désignaler</button>
        <?php } else { ?>
        <button type="button" class="btn btn-outline-success btn-sm">√</button>
        <?php } ?>
      </td>
      <td scope="col">
        <button type="button" class="btn btn-outline-danger btn-sm" onclick="dropComment(<?= $comment['id_c'] ; ?>, 'signaled')">Supprimer</button>
      </td>
    </tr>
    <?php
    }
